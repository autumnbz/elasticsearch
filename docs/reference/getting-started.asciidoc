...
Elasticsearch 是一个高度可扩展的开源全文搜索和分析引擎。它具有存储、搜索以及近实时地快速分析海量数据的功能。它通常被用来作为底层的引擎和技术，助力应用实现复杂的搜索特性和需求。

下面是 Elasticsearch 的几个使用场景:
...
你如果不想要默认名称，可以自定义。为了在管理中确认网络中的服务器对应 Elasticsearch 集群中的哪些节点，节点名称是十分重要的。
