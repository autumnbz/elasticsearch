...
TIP: 历史上使用 `.zip` 在 Windows 上安装 Elasticsearch。
...
Elasticsearch v{version}  的 `.zip` 包从此获取：https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-{version}.zip
...
还可以使用管理器 GUI（ `elasticsearch-service-mgr.exe` ）安装服务，该界面提供详细安装服务过程，包括其状态、启动类型、JVM、启动和停止设置等等。 只需从
...
大多数通过管理 GUI 改变（例如 JVM 设置）的配置需要重启服务才能生效。
...
这十分方便因为不需要建立任何目录来启动 Elasticsearch，卸载 Elasticsearch 也只需要移除 `$ES_HOME` 目录。
